...
class timmCustomAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        self.attn_scores = attn_scores.clone() # Store the pre-softmax attention scores
        attn_pattern = attn_scores.softmax(dim=-1)
        #attn_pattern = self.attn_pattern(attn_pattern) # For hook function
        self.attn_pattern = attn_pattern.clone() # Store the post-softmax attention pattern
